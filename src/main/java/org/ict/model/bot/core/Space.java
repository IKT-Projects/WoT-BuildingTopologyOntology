...
import org.ict.model.bot.core.iface.SpaceIf;
...
public class Space extends Zone implements SpaceIf {
  private static final transient List<URI> defaultType = new ArrayList<>();
  static {
    defaultType.add(URI.create("https://w3id.org/bot#Space"));
  }
  ...
  @Builder
  public Space(URI id, List<URI> atType, String name, List<Zone> containsZone,
  ...
  public void setAtTypeRDF(List<URI> types) {
    ...
    if (filtered.isEmpty()) {
      filtered.add(URI.create("https://w3id.org/bot#Space"));
      setAtType(filtered);
    } else {
      setAtType(filtered);
    }
