...
import org.ict.model.bot.core.iface.BuildingIf;
...
public class Building extends Zone implements BuildingIf {
  private static final transient List<URI> defaultType = new ArrayList<>();
  static {
    defaultType.add(URI.create("https://w3id.org/bot#Building"));
  }
  ...
  @Builder
  public Building(URI id, List<URI> atType, String name, List<Zone> containsZone,
  ...
  public void setAtTypeRDF(List<URI> types) {
    ...
    // FIXME Filter deactivated for now
    // types.stream().filter(u -> !u.equals(URI.create("https://w3id.org/bot#Element")))
    // .collect(Collectors.toList());

    if (filtered.isEmpty()) {
      filtered.add(URI.create("https://w3id.org/bot#Building"));
      setAtType(filtered);
    } else {
      setAtType(filtered);
    }
